...
const Results: React.FC<ResultsProps> = (props) => {
  ...
  return (
    <div className="relative h-full p-4">
      ...
      <table className="w-full rounded-md text-left">
        ...
        <tbody>
          {props.challengeData?.players.map((player) => {
            if (
              props.challengeData?.[player.friendId]?.completed !== null &&
              props.challengeData?.[player.friendId]
            ) {
              ...
              return (
                <tr
                  ...
                  key={player.friendId}
                >
                  <td className="p-3">{player.friendFullName}</td>
                  <td className="p-3">{`${props.challengeData?.[player.friendId]?.completed}`}</td>
                  <td className="p-3">{`${props.challengeData?.[player.friendId]?.success}`}</td>
                  <td className=" text-wrap p-3 text-sm">{props.challengeData?.[player.friendId]?.endTimeStamp ? `${props.challengeData?.[player.friendId]?.guesses}` :"Gave Up"}</td>
                  <td className="p-3">
                    {props.challengeData?.[player.friendId]?.endTimeStamp
      ...
      <button className="absolute bottom-0">Delete</button>
    </div>
  );
};
